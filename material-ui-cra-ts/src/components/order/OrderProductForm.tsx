...
import { TextField, Button, Grid2 as Grid, Box, Autocomplete, Typography } from '@mui/material';
...
interface OrderProductFormProps {
    onSuccess: (product: Product, sizeVaraints: SpecificSizeVariant[], price: number) => void; // This prop should expect a Product as a parameter
    onClose: () => void;
}

const OrderProductForm: React.FC<OrderProductFormProps> = ({ onSuccess, onClose }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (selectedProduct != null) onSuccess(selectedProduct, sizeVariants, price)
    };
    ...
    useEffect(() => {
        ...
        setPrice(p * (selectedProduct?.sell_price ?? 0))
    }, [sizeVariants])
